...
import static io.opentelemetry.auto.instrumentation.opentelemetryapi.v0_2_0.trace.Bridging.toShaded;
import static io.opentelemetry.auto.instrumentation.opentelemetryapi.v0_2_0.trace.Bridging.toUnshaded;
...
class UnshadedHttpTextFormat implements HttpTextFormat<SpanContext> {
  ...
  public <C> SpanContext extract(final C carrier, final HttpTextFormat.Getter<C> getter) {
    ...
    return toUnshaded(getSpan(context).getContext());
  }

  @Override
  public <C> void inject(
      final SpanContext value, final C carrier, final HttpTextFormat.Setter<C> setter) {
    final Context context = withSpan(DefaultSpan.create(toShaded(value)), Context.ROOT);
  ...
  private static class ShadedSetter<C>
    ...
    public void set(final C carrier, final String key, final String value) {
      unshadedSetter.put(carrier, key, value);
    }
  }
}
